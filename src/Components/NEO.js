  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        "https://api.nasa.gov/neo/rest/v1/feed?start_date=2023-02-22&end_date=2023-02-23&api_key=WbRkJzJZ43olavmnr3q6aEvA1JkfCnKppzDeiEKq"
      );
              ...
              <ul>
                {neoData.near_earth_objects[date].map((neo) => (
                  <li key={neo.id}>
                    ...
                    <p>
                      Is hazardous:{" "}
                      {neo.is_potentially_hazardous_asteroid ? "true" : "false"}
                    </p>
                    </div>
                  </li>
                ))}
